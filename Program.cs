...
namespace TelegramDOGs
{
    class Program
    {
        ...
        public static ITelegramBotClient bot = new TelegramBotClient("5384438845:AAG6qrDzwcni1Lk8bBIkXAPCJ2-D7YVG6j0");
                            ...
                            for (int i = 0; i < 10; i++)
                            {
                                if (user.countDog - 1 >= i)
                                {
                                    ...
                                    if (update.CallbackQuery.Data == Convert.ToString(i))
                                    {
                                        ...
                                        string img = @$"D:\C#\TelegramDOGs\image_TelegramDogs\{dog.TypeDogString}.png";
                                        using(var Strem = System.IO.File.OpenRead(img))
                                        {
                                            await botClient.SendPhotoAsync(update.CallbackQuery.From.Id, new Telegram.Bot.Types.InputFiles.InputOnlineFile(Strem));
                                        }
                                    ...
                                    if (update.CallbackQuery.Data == $"Ded_dog{i}")
                                    {
                                        ...
                                        await botClient.DeleteMessageAsync(user.Id, update.CallbackQuery.Message.MessageId);
